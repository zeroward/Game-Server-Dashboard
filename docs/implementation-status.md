...
- [x] Optional Cloudflare Tunnel overlay with a pinned non-root connector, a mounted token file, exact proxy trust, forced HTTPS cookies, and no portal host ports; compatible with the VPN overlay.
...
- [x] `python3 tests/tunnel-smoke.py` passed with disposable Docker resources and a nonworking token on an outbound-blocked network: base/tunnel/VPN Compose merging, missing-hostname rejection, private mount configuration, absent host ports, actual connector token-file parsing/startup, honest offline failure, origin DNS/health, Secure cookies, and no-store responses. Added Go tests passed for HTTPS-to-HTTP proxy login/logout/CSRF and exact proxy-IP trust. The public Cloudflare connection, account setup and real DNS were not tested. The existing browser and packet suites were not rerun for this deployment-only change.
